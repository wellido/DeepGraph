var NodeStates = [ ["A","group"], ["B","group"], ["C","A"], ["D","A"],
    ["E","group3"], ["F","G"], ["G","A"], ["H","A"],
    ["I","group3"], ["J","group"], ["K","G"] ];
...
// var Groups =[['group', ""], ['group2', 'group'], ['group3', 'group']];
...
function dataDecreaseUpdate(group) {
    ...
    for (var i = 0; i < DrawGroups.length; i++) {
        for (var j = 0; j < DelGroups.length; j++) {
            if(DrawGroups[i][1] == DelGroups[j]) {
                if(DelGroups.indexOf(DrawGroups[i][0]) == -1) {
                    DelGroups.push(DrawGroups[i][0]);
                }
            }
        }
    }
    ...
    for (var i = 0; i < OriLength; i++) {
        DelGroups.forEach(function (delgroup) {
            // console.log("DrawNodes[tempIndex]",DrawNodes[tempIndex],"delgroup",delgroup);
